#***************************************
#Arguments
#%1: Test Package (WebPDotNet.CUDA92)
...
function RunTest($BuildTargets)
{
   foreach($BuildTarget in $BuildTargets)
   {
      ...
      # Test
      $WorkDir = Join-Path $WebPDotNet work
      $NugetDir = Join-Path $WebPDotNet nuget
      ...
      $env:WebPDotNet_VERSION = $VERSION
      $env:WebPDotNet_GUI_SUPPORT = 1

      $TestDir = Join-Path $WebPDotNet test | `
                 Join-Path -ChildPath WebPDotNet.Tests

      $TargetDir = Join-Path $WorkDir WebPDotNet.Tests
      ...
      dotnet remove reference "..\..\src\WebPDotNet\WebPDotNet.csproj" > $null
...
$BuildTargets = @()
$BuildTargets += New-Object PSObject -Property @{Target = "cpu";  Architecture = 64; CUDA = 0;   Package = "WebPDotNet"         }
